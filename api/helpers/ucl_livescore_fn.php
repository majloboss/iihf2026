...
function ucl_zapis_naklady(string $model, ?array $modelRow, array $res,
    ...
    try {
        ...
        // Ked nebezal ziadny zapas, live_ids ostava prazdne. Priradenie
        // vsetkym by znamenalo, ze zapas pred vykopom "sledujeme" — v prehlade
        // by mu potom pribudali volania aj hodiny predtym, nez sa zacal hrat.
        ...
        // Vysledky sa uchovavaju kluceny podla game_id, nie ako jeden retazec:
        // v detaile konkretneho zapasu sa ma zobrazit len on, nie cela odpoved.
        $vysledky     = [];
        $vysledkyJson = [];
        foreach ($res['games'] ?? [] as $fsId => $d) {
            if (!isset($watch[$fsId]) || !is_array($d)) continue;
            ...
            // Minuta v hranatych zatvorkach: okruhle by v tomto retazci
            // rozhodili kontrolu vyvazenosti zatvoriek.
            $minuta = !empty($d['minute']) ? ' [' . (int)$d['minute'] . "']" : '';
            $popis  = trim(($d['home_team'] ?? '?') . " $skore "
                           . ($d['away_team'] ?? '?') . $minuta);

            $vysledky[] = $popis;
            $vysledkyJson[(string)$watch[$fsId]] = [
                'popis'  => $popis,
                'skore'  => $skore,
                'stav'   => $d['status'] ?? null,
                'minuta' => !empty($d['minute']) ? (int)$d['minute'] : null,
                'bezi'   => !empty($d['started']) && empty($d['finished']),
            ];
        }
        ...
        db()->prepare(
            ...
            ->execute([
                ...
                $stav,
                $vysledkyJson ? json_encode($vysledkyJson, JSON_UNESCAPED_UNICODE) : null,
            ]);
    } catch (Throwable $e) {
