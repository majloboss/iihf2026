...
if ($method === 'GET') {
    $rows = $pdo->query('SELECT c.country_code, c.country_code2, c.name_sk, c.name_sk_long, c.name_en, c.name_original, c.flag_file, c.flag_file_big, c.is_active, COUNT(t.team_id) AS team_count FROM admin.countries c LEFT JOIN "lm2026-27".teams t ON t.country_code = c.country_code GROUP BY c.country_code, c.country_code2, c.name_sk, c.name_sk_long, c.name_en, c.name_original, c.flag_file, c.flag_file_big, c.is_active ORDER BY c.name_sk, c.country_code')->fetchAll();
    json_ok($rows);
}
...
if ($code2 !== '' && !preg_match('/^[A-Z]{2}$/', $code2)) json_error('Dvojpísmenkový kód štátu musí mať presne 2 písmená', 400);
...
try {
    if ($method === 'POST') {
        $stmt = $pdo->prepare('INSERT INTO admin.countries (country_code, country_code2, name_sk, name_sk_long, name_en, name_original, flag_file, flag_file_big) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING country_code, country_code2, name_sk, name_sk_long, name_en, name_original, flag_file, flag_file_big');
        $stmt->execute([$code, $code2 ?: null, $nameSk, $nameSkLong ?: null, $nameEn, $nameOriginal ?: null, $flagFile ?: null, $flagFileBig ?: null]);
    ...
    if ($method === 'PUT') {
        ...
        if ($oldCode !== $code) {
            ...
            $target->execute([$code]);
            if (!$target->fetch()) {
                $create = $pdo->prepare('INSERT INTO admin.countries (country_code, country_code2, name_sk, name_sk_long, name_en, name_original, flag_file, flag_file_big) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
                $create->execute([$code, $code2 ?: null, $nameSk, $nameSkLong ?: null, $nameEn, $nameOriginal ?: null, $flagFile ?: null, $flagFileBig ?: null]);
            } else {
                $rename = $pdo->prepare('UPDATE admin.countries SET country_code2 = ?, name_sk = ?, name_sk_long = ?, name_en = ?, name_original = ?, flag_file = ?, flag_file_big = ?, updated_at = NOW() WHERE country_code = ?');
                $rename->execute([$code2 ?: null, $nameSk, $nameSkLong ?: null, $nameEn, $nameOriginal ?: null, $flagFile ?: null, $flagFileBig ?: null, $code]);
            }
        ...
        $result = $pdo->prepare('SELECT country_code, country_code2, name_sk, name_sk_long, name_en, name_original, flag_file, flag_file_big FROM admin.countries WHERE country_code = ?');
        $result->execute([$code]);
        $row = $result->fetch();
        $pdo->commit();
        json_ok($row);
