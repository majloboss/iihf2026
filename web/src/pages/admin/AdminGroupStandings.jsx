            ...
            <table className={gsStyles.table}>
                <thead>
                    <tr>
                        ...
                        <th>Z</th><th>V</th>
                        <th className={gsStyles.thOt}>VP</th><th className={gsStyles.thOt}>PP</th>
                        <th>P</th>
                ...
                <tbody>
                    {teams.map((t, i) => (
                        <tr key={t.team} className={i < 4 ? (i === 3 ? gsStyles['last-qualified'] : gsStyles.qualified) : ''}>
                            ...
                            <td>{t.gp}</td><td>{t.w}</td>
                            <td className={gsStyles.tdOt}>{t.otw ?? 0}</td>
                            <td className={gsStyles.tdOt}>{t.otl ?? 0}</td>
                            <td>{t.l}</td>
                            <td>{t.gf}:{t.ga}</td>
                            ...
                            <td className={styles.moveCell}>
                                <button className={styles.btnMove} onClick={() => move(i, -1)}
                                    disabled={finalized || i === 0 || teams[i - 1].pts !== t.pts}
                                    title="Posunúť hore">▲</button>
                                <button className={styles.btnMove} onClick={() => move(i, 1)}
                                    disabled={finalized || i === teams.length - 1 || teams[i + 1].pts !== t.pts}
