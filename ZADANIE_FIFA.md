...
18d. 🟠 **Skupiny — oprava zobrazenia bodov** — dlhé mená hráčov tlačili stĺpce rozpadu bodov (1 a 0) mimo obrazovku. Mobilné buckety 19→17px, tesnejšie paddingy, súčet Body posunutý kúsok doľava. Standings.jsx + Standings.module.css.
18e. 🟠 **Skupiny — meno celé + username max 15** — avatar posunutý doľava (užší # stĺpec, menší padding), meno sa zobrazuje **celé** (zrušený ellipsis), šípka bližšie k menu. Username obmedzený na **15 znakov** (Register.jsx maxLength + complete.php mb_strlen ≤15) — zatiaľ max dĺžka, ktorá sa garantovane zmestí.
